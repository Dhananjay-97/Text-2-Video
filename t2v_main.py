...
class CrossFrameAttnProcessor:
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if not is_cross_attention and self.dual_filter > 0.0:
            ...
            if batch_size != 2:
                attention_probs_rev = attn.get_attention_scores(key, query, attention_mask)

                conf_mat = attention_probs * attention_probs_rev
                ...
                key_orig = attn.head_to_batch_dim(key_orig)
                value_orig = attn.head_to_batch_dim(value_orig)
                
                attention_probs_orig = attn.get_attention_scores(query, key_orig, attention_mask)
                hidden_states_orig = torch.bmm(attention_probs_orig, value_orig)
                del(attention_probs_orig)

        hidden_states = torch.bmm(attention_probs, value)
        # Filtering
        if hidden_states_orig is not None:
            hidden_states = conf_mask * hidden_states + (1 - conf_mask) * hidden_states_orig
...
class T2VPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        motion_field_strength_x: float = 0,
        motion_field_strength_y: float = 0,
